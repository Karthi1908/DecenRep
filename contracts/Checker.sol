...
abstract contract Checker is Context, ERC165, EIP712, IChecker, IERC721Receiver, IERC1155Receiver {
     ...
     */
    function state(uint256 proposalId) public view virtual override returns (ProposalState status) {
        ...
        if (_quorumReached(proposalId)) {
            if(_voteFact(proposalId)) {
                return ProposalState.Fact;
            } else {
                return ProposalState.Fake;
            }
        }
    }
     ...
     */
     function execute(
        ...
        _calldata = abi.encodeWithSignature("factCheckResult(uint256,string)", proposalId, postStatus);
